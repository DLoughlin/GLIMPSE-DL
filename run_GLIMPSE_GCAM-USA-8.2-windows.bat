...
rem Prefer an explicitly configured JAVA_HOME when available.
if defined JAVA_HOME if exist "%JAVA_HOME%\bin\java.exe" set "JAVA_EXE=%JAVA_HOME%\bin\java.exe"

rem Fallback to the legacy bundled JRE path used by earlier distributions.
...
rem Final fallback to java on PATH.
...
"%JAVA_EXE%" -Djava.util.logging.config.file -Dprism.order=sw --module-path "%JAVAFX_LIB%" --add-modules=javafx.controls,javafx.fxml --add-exports=javafx.base/com.sun.javafx.runtime=ALL-UNNAMED --add-exports=javafx.graphics/com.sun.javafx.tk=ALL-UNNAMED -jar .\GLIMPSE-ScenarioBuilder\GLIMPSE-ScenarioBuilder.jar -options options_GCAM-USA-8.2-windows.txt
